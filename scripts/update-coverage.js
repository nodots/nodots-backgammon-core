    ...
    (error, stdout, stderr) => {
      ...
      if (parts.length >= 5) {
        ...
        const badges = [
          ...
          `![Lines](https://img.shields.io/badge/Lines-${linesPct}%25-${getColorForCoverage(
            linesPct
          )}?style=flat-square)`,
        ]
...
function updateReadme(coverageTable) {
  ...
  // Find the coverage section in the README
  const coverageStartMarker = '<!-- COVERAGE-START -->'
  const coverageEndMarker = '<!-- COVERAGE-END -->'

  const startIndex = content.indexOf(coverageStartMarker)
  const endIndex = content.indexOf(coverageEndMarker)

  if (startIndex === -1 || endIndex === -1) {
    console.error('Could not find coverage markers in README.md')
    return
  }

  const beforeCoverage = content.substring(
    0,
    startIndex + coverageStartMarker.length
  )
  const afterCoverage = content.substring(endIndex)

  const newContent =
    beforeCoverage + '\n' + coverageTable + '\n' + afterCoverage

  fs.writeFileSync(readmePath, newContent)
  console.log('README.md updated with new coverage badges')
}

// Run the script
runCoverageAndParseOutputAsync((badges) => {
  if (badges) {
    updateReadme(badges)
  } else {
    console.error('Failed to generate coverage badges')
  }
})
