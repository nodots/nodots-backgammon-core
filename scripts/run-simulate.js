// Lightweight launcher to run the TypeScript simulator without npm
// Usage: from packages/core, run: node scripts/run-simulate.js [maxTurns]
try {
  require('ts-node/register')
} catch (e) {
  console.error('Failed to load ts-node/register. Ensure dev deps are installed in packages/core.')
  throw e
}

const maxTurns = process.argv[2] ? parseInt(process.argv[2], 10) : undefined
require('../src/scripts/simulate.ts')
  .runSimulation(maxTurns)
  .catch((err) => {
    console.error('Simulation failed:', err && err.message ? err.message : err)
    process.exit(1)
  })

