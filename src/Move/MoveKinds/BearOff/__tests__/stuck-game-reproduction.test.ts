...
describe('Stuck Game Reproduction - Game ID f9521db8-0e63-4c16-a5ac-86d77a7132e2', () => {
  ...
  it('should verify the stuck game fix prevents games from getting stuck', () => {
    ...
    console.log(
      'Point 6:',
      board.points.find((p) => p.position.clockwise === 6)?.checkers.length ||
        0,
      'white checkers'
    )
    console.log(
      'Point 5:',
      board.points.find((p) => p.position.clockwise === 5)?.checkers.length ||
        0,
      'white checkers'
    )
    console.log(
      'Point 4:',
      board.points.find((p) => p.position.clockwise === 4)?.checkers.length ||
        0,
      'white checkers'
    )
    console.log(
      'Point 3:',
      board.points.find((p) => p.position.clockwise === 3)?.checkers.length ||
        0,
      'white checkers'
    )
    console.log(
      'Point 2:',
      board.points.find((p) => p.position.clockwise === 2)?.checkers.length ||
        0,
      'white checkers'
    )
    console.log(
      'Point 1:',
      board.points.find((p) => p.position.clockwise === 1)?.checkers.length ||
        0,
      'white checkers'
    )
    ...
    while (moveCount < maxMoves) {
      ...
      const readyMoves = Array.from(movingPlay.moves).filter(
        (m) => m.stateKind === 'ready'
      )
      ...
      if (readyMoves.length === 0) {
        ...
        console.log(
          '- Point 6:',
          board.points.find((p) => p.position.clockwise === 6)?.checkers
            .length || 0
        )
        console.log(
          '- Point 5:',
          board.points.find((p) => p.position.clockwise === 5)?.checkers
            .length || 0
        )
        console.log(
          '- Point 4:',
          board.points.find((p) => p.position.clockwise === 4)?.checkers
            .length || 0
        )
        ...
        Array.from(movingPlay.moves).forEach((move, idx) => {
          console.log(
            `  Move ${idx + 1}: ${move.stateKind} (die=${move.dieValue}, kind=${move.moveKind})`
          )
        })
      ...
      if (moveToExecute.possibleMoves.length > 0) {
        ...
        console.log(
          `Origin from possibleMoves: ${validOrigin.kind} (${validOrigin.id.substring(0, 8)})`
        )
      } else if (moveToExecute.origin) {
        validOrigin = moveToExecute.origin
        console.log(
          `Origin from move.origin: ${validOrigin.kind} (${validOrigin.id.substring(0, 8)})`
        )
      } else {
    ...
    executedMoves.forEach((move, idx) => {
      console.log(
        `Move ${idx + 1}: die=${move.dieValue}, kind=${move.moveKind}, success=${move.success}`
      )
    })
    ...
    console.log(
      '- Checkers remaining on board:',
      board.points.reduce(
        (sum, point) =>
          sum + point.checkers.filter((c) => c.color === 'white').length,
        0
      )
    )
  ...
  it('should investigate possible moves generation after each move in doubles scenario', () => {
    ...
    const initialPossibleMoves = Board.getPossibleMoves(
      board,
      player,
      6 as BackgammonDieValue
    )
    console.log(
      'Initial possible moves with die 6:',
      initialPossibleMoves.length
    )

    initialPossibleMoves.forEach((move, idx) => {
      console.log(
        `  Initial move ${idx + 1}: ${move.origin.kind} -> ${move.destination.kind}`
      )
    ...
    for (let moveNum = 1; moveNum <= 4; moveNum++) {
      ...
      if (possibleMoves.length === 0) {
        ...
        for (let point = 6; point >= 1; point--) {
          const checkers =
            currentBoard.points.find((p) => p.position.clockwise === point)
              ?.checkers.length || 0
          if (checkers > 0) {
        ...
        console.log(
          `  Off: ${currentBoard.off.clockwise.checkers.length} white checkers`
        )
      ...
      console.log(
        `Executing move ${moveNum}: ${moveToExecute.origin.kind} -> ${moveToExecute.destination.kind}`
      )
      ...
      console.log(
        `  Point 6: ${currentBoard.points.find((p) => p.position.clockwise === 6)?.checkers.length || 0}`
      )
    }
  })
})
