...
describe('Bear-off possibleMoves Recalculation Bug', () => {
  ...
  it('should correctly calculate possibleMoves after each bear-off move execution', () => {
    ...
    console.log(
      'Expected: 3 bear-off moves possible, 1 move should be no-move or use different point'
    )

    // Test initial possibleMoves
    console.log('\n--- Initial possibleMoves calculation ---')
    const initialMoves = Board.getPossibleMoves(
      board,
      player,
    ...
    initialMoves.forEach((move, idx) => {
      console.log(
        `  Move ${idx + 1}: ${move.origin.kind} -> ${move.destination.kind}`
      )
    ...
    console.log(
      'Point 6 checkers after 2nd move:',
      board.points.find((p) => p.position.clockwise === 6)?.checkers.length || 0
    )
    console.log('Off checkers:', board.off.clockwise.checkers.length)

    const movesAfter2 = Board.getPossibleMoves(
      board,
      player,
      6 as BackgammonDieValue
    )
    ...
    console.log(
      'Point 6 checkers after 3rd move:',
      board.points.find((p) => p.position.clockwise === 6)?.checkers.length || 0
    )
    console.log('Off checkers:', board.off.clockwise.checkers.length)

    // This is where the bug likely occurs - no more moves with die 6
    const movesAfter3 = Board.getPossibleMoves(
      board,
      player,
      6 as BackgammonDieValue
    )
    console.log(
      'Possible moves after 3rd move (THIS IS THE BUG):',
      movesAfter3.length
    )

    if (movesAfter3.length === 0) {
      console.log(
        '🚨 BUG CONFIRMED: No possible moves for 4th die after 3 bear-off moves'
      )
      console.log('Current board state:')
      for (let point = 6; point >= 1; point--) {
        const checkers =
          board.points
            .find((p) => p.position.clockwise === point)
            ?.checkers.filter((c) => c.color === 'white').length || 0
        if (checkers > 0) {
      ...
      const checkersOnPoint1 =
        board.points
          .find((p) => p.position.clockwise === 1)
          ?.checkers.filter((c) => c.color === 'white').length || 0
      if (checkersOnPoint1 > 0) {
        console.log(
          '❌ RULE VIOLATION: Should be able to use die 6 to bear off from point 1'
        )
        console.log(
          '   (Higher die rule: can use higher number when no checkers on higher points)'
        )
      }
    }
  ...
  it('should test the higher die rule specifically for bear-off', () => {
    ...
    console.log(
      'Testing die value 6 (should bear off from point 3 using higher die rule)'
    )
    ...
    possibleMoves.forEach((move, idx) => {
      ...
      if (move.origin.kind === 'point') {
        ...
        if (originPosition === 3) {
          console.log(
            '    ✅ Correctly using higher die rule to bear off from point 3'
          )
        }
      }
    })
    ...
    const bearOffMove = possibleMoves.find(
      (move) =>
