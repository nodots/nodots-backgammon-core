...
export class BasicAIPlugin implements BackgammonAIPlugin {
  ...
  version = '1.0.0'
  description =
    'Basic AI with heuristic-based move selection using direction-based positioning'
  ...
  async shouldOfferDouble(
    ...
    const opponentPipCount = PositionAnalyzer.calculatePipCount(
      gameState,
      opponent
    )
  ...
  private selectBeginnerMove(
    possibleMoves: any[],
    gameState: BackgammonGame
  ): any {
    // CRITICAL FIX: Even beginners should prioritize bear-off moves to prevent timeout bugs
    // Check if any bear-off moves are available
    const bearOffMoves = possibleMoves.filter(
      (move) => move.destination.kind === 'bear-off'
    )

    if (bearOffMoves.length > 0) {
      // Always prioritize bear-off moves to avoid timeout scenarios
      return bearOffMoves[Math.floor(Math.random() * bearOffMoves.length)]
    }

    // For non-bear-off scenarios, use simple random selection
