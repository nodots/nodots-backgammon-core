...
export class XGConverter {
  ...
  public static async xgGameToNodotsGame(
    ...
    // Player.initialize(color, direction, stateKind, isRobot, userId?, pipCount?, id?)
    ...
    // Check if game actually finished (winner is defined and not 0)
    const isGameCompleted = xgGame.winner !== undefined && xgGame.winner !== null && xgGame.pointsWon > 0

    // Determine winner - XG uses 1 or 2 for player number
    const winnerPlayer = xgGame.winner === 1 ? player1 : (xgGame.winner === 2 ? player2 : null)
    const loserPlayer = xgGame.winner === 1 ? player2 : (xgGame.winner === 2 ? player1 : null)

    // Add XG-specific metadata
    // Only mark as completed if the game actually finished (has a winner with points won)
    // Using type assertion since XG imports don't fit the strict game type hierarchy
    const gameWithMetadata = {
      ...game,
      stateKind: isGameCompleted ? 'completed' : 'moving', // Unfinished games are in progress
      ...(isGameCompleted && winnerPlayer ? { winner: winnerPlayer.id } : {}),
      activeColor: winnerPlayer?.color || player1.color,
      activePlayer: winnerPlayer || player1,
      inactivePlayer: loserPlayer || player2,
      metadata: {
    ...
    } as Partial<BackgammonGame> // XG imports bypass strict type checks
